...
public class ManagerQR {
	...
	public void regitrarJornada(Integer idJornada, Integer idEvento, String descripcionJornada,Date fechajornada, String lugarJornada) {
	...
	public void publicarEvento(Integer tipoEvnt, Integer idEvnt, String nombreEvento, String descripcionEvnt,
			Date fechaInicioEvnt, Date fechaFinEvnt, String direccionEvnt) throws MiExcepcion {
		Date fechaActual = new Date();
		long horaActual = fechaActual.getTime();
		long horaFin = fechaFinEvnt.getTime();
		...
		if (fechaInicioEvnt.before(fechaFinEvnt) || fechaInicioEvnt.equals(fechaFinEvnt) && horaInicio <= horaFin) {
			if (fechaActual.before(fechaInicioEvnt)
				...
				nuevoEvento.setDireccionEvnt(direccionEvnt);
				nuevoEvento.setTbltpevento(tipoevento);
				nuevoEvento.setPublicado(Boolean.TRUE);
				em.merge(nuevoEvento);
			} else {
				throw new MiExcepcion("Fecha inicio incorrecta");
			}
	...
	public List<Tblevento> listaEventosGuardados(Integer idUsuario) {
		Query q = em.createQuery("select u from Tblevento u where u.tblusuario.idUsuario=:idusuario and u.publicado is null")
				.setParameter("idusuario", idUsuario);
		return q.getResultList();
	}
	@SuppressWarnings("unchecked")
	public List<Tblevento> listaEventosPublicados(Integer idUsuario) {
		Query q = em.createQuery("select u from Tblevento u where u.tblusuario.idUsuario=:idusuario and u.publicado is true")
				.setParameter("idusuario", idUsuario);
		return q.getResultList();
	}
	@SuppressWarnings("unchecked")
	public List<Tblevento> listaEventosTerminados(Integer idUsuario) {
		Query q = em.createQuery("select u from Tblevento u where u.tblusuario.idUsuario=:idusuario and u.publicado is false")
				.setParameter("idusuario", idUsuario);
		return q.getResultList();
	}
	...
	public boolean comprobarUsuario(String correoUsuario, String claveUsuario) throws MiExcepcion {
		...
		if (u.getClaveUsuario().equals(claveUsuario))
			return true;
		throw new MiExcepcion("Contraseña no válida.");
	}
